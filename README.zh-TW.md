...
```text
請閱讀並遵循這份安裝 runbook：
https://raw.githubusercontent.com/Nanako0129/remora-cc/v0.1.1/install/AGENT-INSTALL.md
...
```bash
git clone --branch v0.1.1 --depth 1 https://github.com/Nanako0129/remora-cc.git
cd remora-cc
./install.sh
```
...
Remora 是把一項範圍明確的技術包裝成可審查的工具，並不宣稱發明了 multi-agent routing。[pilotfish](https://github.com/Nanako0129/pilotfish) 奠定依角色分工的 orchestration 模式；Anthropic 提供動態 session agents 與自訂 LLM gateway 的官方機制；[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) 則提供協定橋接。[Overdeck](https://github.com/eltmon/overdeck) 是規模更完整的 orchestration 平台，也能管理自己的 CLIProxyAPI sidecar。Remora 選擇相反的產品邊界：只替既有 gateway 提供一個可稽核的 launcher，且不改動原生 Claude state。
