...
啟動時，Remora 會唯讀查詢 gateway 的 Codex-compatible model catalog，在 main 與所有 agent model 中取最小 context window，再比照 Codex CLI：95% 作為 effective context，90% 觸發自動 compact。目前 372K catalog 對應 353.4K effective context 與 334.8K compact trigger。Remora 只把 raw window 與 compact 百分比注入 Claude Code child；catalog 查詢失敗或資料不完整時採保守 fallback，不修改 CLIProxyAPI，也不影響原生 Claude。
...
| `Your input exceeds the context window` | Claude Code 累積內容超過 gateway provider 的實際 ceiling | 執行 `remora doctor --online`；保持 context discovery 開啟，或降低 `[context].fallback_window` |
