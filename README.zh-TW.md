...
remora 只替一個 child `claude` process 注入 `--agents` JSON、session-only 編排補充指令與 gateway 環境變數。Claude Code 官方把 `--agents` 定義為只存在於當前 session 的 agent source；它可以暫時覆蓋同名的 project/user agent，但不會寫入磁碟。編排補充指令會先套用 dispatch brake：規格未穩定、ownership 重疊，以及探索、runtime trace、根因與實作共用同一條 context 的工作，都留在 main session。通過後再比較整體淨效益；即使直接做稍快，只要範圍明確的 Luna worker 能實質節省 Sol 額度，仍可委派。少量 repo 檔案的唯讀掃描預設直接做；穩定的多檔重複工作、真正大型的獨立掃描與 fresh verification 仍保有明確委派路徑。
...
| remora 會覆寫另外安裝的 delegation skill 嗎？ | 不保證。通用 skill 會在對話較後面注入，可能提供衝突的 routing 指引。公開 v0.1.8 實驗觀察到 [baton-dispatch](https://github.com/cablate/baton) 讓 GPT-5.6 Sol 無視 remora 的規模 gate，對小型掃描 fan-out。建議只啟用一個 orchestration authority，或替 skill 加 project-local compatibility rule；remora 不會暗中停用使用者 skills。 |
